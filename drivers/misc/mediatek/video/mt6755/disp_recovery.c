...
extern void himax_ic_reset(uint8_t loadconfig,uint8_t int_off);/* [lichenggang]Add for esd*/
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		/* [yanlin start] Use to recovery lcm when in double-click wakeup mode */
		if (atomic_read(&esd_check_failed) != 0) {
			atomic_set(&esd_check_failed, 0);
		}
		/* [yanlin end] */
		...
		if (esd_check_enable) {
			...
			do {
				...
				if (ret == 1) {
					...
					/* [yanlin start] Use to recovery lcm when in double-click wakeup mode */
					atomic_set(&esd_check_failed, 1);
					primary_display_esd_recovery();
					atomic_set(&esd_check_failed, 0);
					/* [lichenggang]Add for esd*/
