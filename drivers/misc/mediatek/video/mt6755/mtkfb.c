...
/* [yanlin start] Add lcd info for TP910 */
...
int mtkfb_get_lcd_id_by_aux(void)
{
	...
	DISPERR("[lcd_auxadc_get_data(AUX_IN2_NTC)]: ret=%d, data[0]=%d, data[1]=%d\n", ret, data[0], data[1]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	unsigned char panel_ic[4][PANEL_INFO_MAX] =   {{"s61350"},{"hx8394f"},{"hx83102b"},{"unknown"}};
	unsigned char panel_type[4][PANEL_INFO_MAX] = {{"truly"}, {"booyi"},  {"truly"},   {"unknown"}};
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = mtkfb_get_lcd_id_by_aux();
	// [tanghonghui start] Modify lcm vendor for TP910
	if (ret==R61350_VOL) { // for 910 is truly
		vendor = 0;
	} else if (ret== HX8394F_VOL){ //for 910 is booyi
		vendor = 1;
	}
	// [tanghonghui end]
#else
	vendor = 0; // TP913, just one lcm hx83102b now
#endif
