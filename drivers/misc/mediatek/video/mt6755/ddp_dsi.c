...
int ddp_dsi_set_lcm_utils(DISP_MODULE_ENUM module, LCM_DRIVER *lcm_drv)
{
	...
	utils->set_gpio_lcd_enn_bias = lcd_enn_bias_setting;	// [yanlin] add
	utils->set_gpio_lcd_tp_rst = lcd_tp_rst_setting;	// [yanlin] add
#endif
#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			t0.Data_ID =
			    (t0.Data0 <
			     0xFF) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;/* [lichenggang]change for esd，change 0xB0 to 0xFF*/
			t0.Data1 = 0;
