...
//#define CONFIG_TPLINK_PRODUCT_TP910
/******************************************************************************
 * Debug configuration
******************************************************************************/
	 ...
	 {SENSOR_DRVNAME_IMX135_MIPI_RAW,
	  {
	   ...
	   {RST, Vol_High, 0}
         ...
         {SENSOR_DRVNAME_OV8856_TP913_SUNWIN,
          {
           {PDN, Vol_Low, 1},
           {RST, Vol_Low, 1},
           {DOVDD, Vol_1800, 1},
         ...
         {SENSOR_DRVNAME_OV8856_TP913_SHINE,
          {
           {PDN, Vol_Low, 1},
           {RST, Vol_Low, 1},
           {DOVDD, Vol_1800, 1},
         ...
         {SENSOR_DRVNAME_OV13856_TP913_RAW,
          {
           {PDN, Vol_Low, 1},
           {RST, Vol_Low, 1},
           {DOVDD, Vol_1800, 1},
...
static int maincam_status = 0;
static int subcam_status = 0;
#endif
...
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
int cam_avdd_pin[MAX_CAM_NUM] = {GPIO20 | 0x80000000, GPIO20 | 0x80000000};
#else
...
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
int cam_dvdd_pin = GPIO21 | 0x80000000;
BOOL cam_dvdd_pin_enable(int status)
{

        if (mt_set_gpio_mode(cam_dvdd_pin, GPIO_MODE_00)) {
                PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (DVDD)\n");
                return FALSE;
        }

        if (mt_set_gpio_dir(cam_dvdd_pin, GPIO_DIR_OUT)) {
                PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (DVDD)\n");
                return FALSE;
        }

        if (mt_set_gpio_out(cam_dvdd_pin, status)) {
                PK_DBG("[CAMERA SENSOR] set gpio failed!! (DVDD)\n");
                return FALSE;
        }
	...
	if (pwInfo.PowerType == AVDD) {
		...
		if (pinSetIdx == 2) {
...
#if defined(CONFIG_TPLINK_PRODUCT_TP904) || defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP913)
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				/* PK_DBG("[CAMERA SENSOR] Main camera VCAM_D power on"); */
				if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
					PK_ERR("[CAMERA SENSOR] Fail to enable digital power\n");
			...
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				/* PK_DBG("[CAMERA SENSOR] Main camera VCAM_D power on"); */
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
				if (TRUE != cam_dvdd_pin_enable(GPIO_OUT_ONE)) {
					PK_ERR("[CAMERA SENSOR] Fail to power up DVDD_EN\n");
					return FALSE;
				}
#else
				if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
	...
	if (pwInfo.PowerType == AVDD) {
		if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (camIdx == 2) {
				...
				if (camIdx == 0 && subcam_status == 1) {
					pr_err("[TPLINK] sub cam is power on, avdd can not power down\n");
					return TRUE;
				}

				if (camIdx == 1 && maincam_status == 1) {
					pr_err("[TPLINK] main cam is power on, avdd can not  power down\n");
					return TRUE;
				}
			...
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
				if (TRUE != cam_dvdd_pin_enable(GPIO_OUT_ZERO)) {
		       ...
		       char *mode_name)
{
	...
	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_OV13856_TP913_RAW) == 0)) {
		if (pinSetIdx != 0)
			goto _kdCISModulePowerOn_exit_;
	}

	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_OV8856_TP913_SUNWIN) == 0 || strcmp(currSensorName, SENSOR_DRVNAME_OV8856_TP913_SHINE) == 0 ||
            strcmp(currSensorName, SENSOR_DRVNAME_GC8034_MIPI_RAW) == 0)) {
		if (pinSetIdx != 1)
			goto _kdCISModulePowerOn_exit_;
	}

	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_HI556_MIPI_RAW) == 0)) {
		if (pinSetIdx != 2)
			goto _kdCISModulePowerOn_exit_;
	}
	...
	if(has_mipi_switch){
		...
		for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
			if (currSensorName && (PowerOnList.PowerSeq[pwListIdx].SensorName != NULL)
				...
				if (0 == strncmp("ov13856", currSensorName, 7)) {
					camIdx = 0;
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
					maincam_status = 1;
#endif
				} else if ((0 == strncmp("ov8856", currSensorName, 6)) || (0 == strncmp("gc8034", currSensorName, 6))) {
					camIdx = 1;
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
					subcam_status = 1;
#endif
