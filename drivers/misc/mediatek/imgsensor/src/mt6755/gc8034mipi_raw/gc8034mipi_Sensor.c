...
static void gc8034_gcore_identify_otp(void)
{
	...
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xfc, 0xee);	
	if (gc8034_otp_info.module_id != MODULE_SUNWIN_MID) {
            LOG_INF("mid is wrong, expect is 0x%x, but read is 0x%x\n", MODULE_SUNWIN_MID, gc8034_otp_info.module_id);
        }else{
            front_camera_module_current = MODULE_GC8034_SUNWIN;
        }
}
