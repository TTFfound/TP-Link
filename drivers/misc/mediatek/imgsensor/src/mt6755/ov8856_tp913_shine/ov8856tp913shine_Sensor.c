...
#include "ov8856tp913shine_Sensor.h"

#define PFX "OV8856TP913SHINE"
#define FLIP
#define MIRROR
#define OV8856R2AOTP
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856_TP913_SHINE_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int RG_Ratio_Typical= 276;
static int BG_Ratio_Typical= 329;
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0300, 0x04);
	write_cmos_sensor(0x0302, 0x5b);// [linyimin] Mofify mipi freq for ratio
	write_cmos_sensor(0x0303, 0x00);
	write_cmos_sensor(0x030b, 0x02);
	write_cmos_sensor(0x030d, 0x3b);
	write_cmos_sensor(0x3501, 0x4c);
	...
	write_cmos_sensor(0x382a, 0x03);
	write_cmos_sensor(0x4009, 0x05);
	write_cmos_sensor(0x4837, 0x15);// [linyimin] Mofify mipi freq for ratio
	write_cmos_sensor(0x4601, 0x80);
	write_cmos_sensor(0x5795, 0x00);
	write_cmos_sensor(0x5796, 0x10);
	write_cmos_sensor(0x5797, 0x10);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 2;
...
#ifdef OV8856R2AOTP
				read_otp(&otp_ptr);
				if (otp_ptr.module_integrator_id != MODULE_SHINE_MID) {
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("mid is wrong, expect is 0x%x, but read is 0x%x\n", MODULE_SHINE_MID, otp_ptr.module_integrator_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
#endif
/* [huangxiancong end] Add read and apply otp function */
				front_camera_module_current = MODULE_OV8856_SHINE;
				return ERROR_NONE;
			}
...
UINT32 OV8856_TP913_SHINE_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
