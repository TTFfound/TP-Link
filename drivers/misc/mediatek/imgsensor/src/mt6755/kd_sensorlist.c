...
unsigned char main2_eeprom_buf[2048] = {0}; /* [huangxiancong] Support Main2 OTP */
unsigned char oqc_eeprom_buf[2048] = {0}; /* [huangxiancong] Support Main2 OTP */
#endif
...
int iMultiReadReg(u16 a_u2Addr , u8 *a_puBuff , u16 i2cId, u8 number)
{
    ...
    else if (gI2CBusNum == SUPPORT_I2C_BUS_NUM2) {
    spin_lock(&kdsensor_drv_lock);
...
int iBurstWriteReg_multi(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length)
{
    ...
    retry = 3;
    do {
        ret = i2c_master_send(g_pstI2Cclient3, (u8 *)((uintptr_t)phyAddr),
        bytes == transfer_length ? transfer_length : ((bytes/transfer_length)<<16)|transfer_length);
        retry--;
        if ((ret&0xffff) != transfer_length) {
        PK_ERR("Error sent I2C ret = %d\n", ret);
...
static int CAMERA_HW_i2c_probe3(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    g_pstI2Cclient3->timing = 400;/* 100k */
...
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
static ssize_t  main2_otp_read(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
    int count;
    count = copy_to_user((void __user *)data, main2_eeprom_buf, sizeof(main2_eeprom_buf));
    return count;
}

static ssize_t  main2_otp_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    return count;
}

static  struct file_operations fcamera_proc_fops4 = {
    .read  = main2_otp_read,
    .write = main2_otp_write,
};

static ssize_t  oqc_otp_read(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
    int count;
    count = copy_to_user((void __user *)data, oqc_eeprom_buf, sizeof(oqc_eeprom_buf));
    return count;
}

static ssize_t  oqc_otp_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    return count;
}
...
struct camera_module support_camera_modules[] = {
	...
	[MODULE_OV13855_TRULY]     = {"ov13855", "Truly",      "1.0.0"},
	[MODULE_OV8856_SUNWIN]     = {"ov8856", "Sunwingroup", "1.0.0"},
	[MODULE_OV8856_SHINE]      = {"ov8856", "Shinetech",   "1.0.0"},
	[MODULE_HI556_TRULY]       = {"hi556",  "Truly",       "1.0.0"},
	[MODULE_GC8034_SUNWIN]     = {"gc8034",  "Sunwingroup","1.0.0"},
};
...
/* FIX-ME: linux-3.10 procfs API changed */
    ...
    /* [huangxiancong start] Support Main2 OTP */
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
    memset(main2_eeprom_buf, 0, sizeof(main2_eeprom_buf));
    proc_create("driver/main2eeprom", 0, NULL, &fcamera_proc_fops4);

    memset(oqc_eeprom_buf, 0, sizeof(oqc_eeprom_buf));
    proc_create("driver/oqceeprom", 0, NULL, &fcamera_proc_fops5);
