...
#define per_frame 1
extern unsigned char main2_eeprom_buf[2048]; /* [huangxiancong] Support Main2 OTP */
static int is_cap2_setting = 0;

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI556_SENSOR_ID,
	.checksum_value = 0x55e2a82f,
	.pre = {
		.pclk = 192000000,				//record different mode's pclk
	...
	.hs_video = {
        .pclk = 192000000,
        .linelength = 2816,				//record different mode's linelength
		.framelength = 568,				//record different mode's framelength
		.startx = 0,
		.starty = 0,
	...
	.custom2 = {                      //capture for PIP 24fps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 192000000,
		.linelength = 2816,
		.framelength = 2840,
		.startx = 0,
	...
	.isp_driving_current = ISP_DRIVING_8MA, // [huangxiancong] Change to 8MA
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (is_cap2_setting == 0 && imgsensor.autoflicker_en) {
...
write_cmos_sensor(0x0f30, 0x7825);
...
write_cmos_sensor(0x0006, 0x08E0);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	{
                LOG_INF("capture_setting fps = 240\n");
                write_cmos_sensor(0x0b0a, 0x8252);
                write_cmos_sensor(0x0f30, 0x7825);
                write_cmos_sensor(0x0f32, 0x7067);
                write_cmos_sensor(0x004a, 0x0100);
                ...
                write_cmos_sensor(0x0030, 0x1111);
                write_cmos_sensor(0x0032, 0x1111);
                write_cmos_sensor(0x0006, 0x0b18);
                write_cmos_sensor(0x0a22, 0x0000);
                write_cmos_sensor(0x0a12, 0x0a20);
                write_cmos_sensor(0x0a14, 0x0798);
                write_cmos_sensor(0x003e, 0x0000);
	...
	else
	{
		...
		write_cmos_sensor(0x0030, 0x1111);
		write_cmos_sensor(0x0032, 0x1111);
		write_cmos_sensor(0x0006, 0x11C0);
		write_cmos_sensor(0x0a22, 0x0000);
		write_cmos_sensor(0x0a12, 0x0a20);
		write_cmos_sensor(0x0a14, 0x0798);
		write_cmos_sensor(0x003e, 0x0000);
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x0006, 0x0238);
...
/* [huangxiancong start] Support Main2 OTP */
static void read_otp(void)
{
	...
	if (main2_eeprom_buf[0] == 0x02)
	{
		LOG_ERR("Hi556 otp has read, not need to read\n");
		return;
	}

	write_cmos_sensor_8(0x0a02,0x01); // Fast sleep mode
	...
	// read Module info
	write_cmos_sensor_8(0x010a,0x04);
	write_cmos_sensor_8(0x010b,0x01);
	...
	if (addr != 0) {
		for (i=0; i<17; i++) {
			write_cmos_sensor(0x010a, addr+i);
			write_cmos_sensor_8(0x0102,0x01);
			main2_eeprom_buf[i] = read_cmos_sensor(0x0108);
			if (i != 16)
				sum += main2_eeprom_buf[i];
		}

		if (main2_eeprom_buf[16] == (sum % 255 + 1)) {
                        LOG_INF("hi556 module info otp checksum is right\n");
		} else {
			LOG_ERR("hi556 module info otp checksum is wrong\n");
		}
	}

	// read AWB info
	write_cmos_sensor_8(0x010a,0x04);
	write_cmos_sensor_8(0x010b,0x35);
	write_cmos_sensor_8(0x0102,0x01);
	otp_flag = read_cmos_sensor(0x0108);
	if (otp_flag == 0x01) {
		addr = 0x0436;
	...
	if (addr != 0) {
		for (i=0; i<30; i++) {
			write_cmos_sensor(0x010a, addr+i);
			write_cmos_sensor_8(0x0102,0x01);
			main2_eeprom_buf[i+17] = read_cmos_sensor(0x0108);
			if (i != 29)
				sum += main2_eeprom_buf[i+17];
		}

		if (main2_eeprom_buf[46] == (sum % 255 + 1)) {
			LOG_INF("hi556 awb otp checksum is right\n");
		} else {
			LOG_ERR("hi556 awb otp checksum is wrong\n");
		}
	}

	// read LSC info
	write_cmos_sensor_8(0x010a,0x04);
	write_cmos_sensor_8(0x010b,0xb5);
	write_cmos_sensor_8(0x0102,0x01);
	otp_flag = read_cmos_sensor(0x0108);
	if (otp_flag == 0x01) {
		addr = 0x04b6;
	...
	if (addr != 0) {
		for (i=0; i<1869; i++) {
			write_cmos_sensor(0x010a, addr+i);
			write_cmos_sensor_8(0x0102,0x01);
			main2_eeprom_buf[i+47] = read_cmos_sensor(0x0108);
                        if (i != 1868)
                                sum += main2_eeprom_buf[i+47];
                }

                if (main2_eeprom_buf[1915] == (sum % 255 + 1)) {
                        LOG_INF("hi556 lsc otp checksum is right\n");
                } else {
                        LOG_ERR("hi556 awb otp checksum is wrong\n");
                }
        }
}
/* [huangxiancong end] Support Main2 OTP */
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	/*else if (imgsensor.current_fps == imgsensor_info.cap2.max_framerate) // 24fps
	{
