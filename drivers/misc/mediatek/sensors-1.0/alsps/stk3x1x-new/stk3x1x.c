...
//#define CONFIG_TPLINK_PRODUCT_TP910 tp910
...
#elif defined(CONFIG_TPLINK_PRODUCT_TP913)
// [wuzhe] modified output data rate of als
#define ALSCTRL_REG_VALUE 0x39
#else
#define ALSCTRL_REG_VALUE 0x39
#endif
...
/* [yanlin start] modified calibration */
	...
	#define STK_LT_N_CT	63
	#define STK_HT_N_CT	101
	#define PS_THDH_LIMITED 2200
	#define PS_THDL_LIMITED 2000
	#define PS_MAX_THD_RANGE 1900
#else
	#define STK_MAX_MIN_DIFF	200
...
static int winfac = 195;
/* [yanlin end] */
#elif defined(CONFIG_TPLINK_PRODUCT_TP913)
// [wuzhe] modified resolution of als
static int winfac = 148;
#else
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	// [yanlin] Add for als adjust
	als_data = (als_data * winfac / 100);
...
#if 0
		if (obj->ps < 50) {
			atomic_set(&obj->ps_high_thd_val, obj->ps + 101);
			atomic_set(&obj->ps_low_thd_val, obj->ps + 61);
		} else if (obj->ps < 200) {
			atomic_set(&obj->ps_high_thd_val, obj->ps + 75);
			atomic_set(&obj->ps_low_thd_val, obj->ps + 43);
		} else {
			atomic_set(&obj->ps_high_thd_val, obj->ps + 50);
			atomic_set(&obj->ps_low_thd_val, obj->ps + 30);
		}
#else
		atomic_set(&obj->ps_high_thd_val, obj->ps + obj->stk_ht_n_ct);
		atomic_set(&obj->ps_low_thd_val, obj->ps + obj->stk_lt_n_ct);
...
#if 0
	if (obj->crosstalk < 50) {
		obj->ps_high_thd_boot = obj->crosstalk + 101;
		obj->ps_low_thd_boot = obj->crosstalk + 61;
	} else if (obj->crosstalk < 200) {
		obj->ps_high_thd_boot = obj->crosstalk + 75;
		obj->ps_low_thd_boot = obj->crosstalk + 43;
	} else if (obj->crosstalk < PS_MAX_THD_RANGE) {
		obj->ps_high_thd_boot = obj->crosstalk + 50;
		obj->ps_low_thd_boot = obj->crosstalk + 30;
	} else {
		obj->ps_high_thd_boot = PS_THDH_LIMITED;
		obj->ps_low_thd_boot = PS_THDL_LIMITED;
	}
#else
...
#if 0
				if (obj->ps < 50) {
					atomic_set(&obj->ps_high_thd_val, obj->psi + 101);
					atomic_set(&obj->ps_low_thd_val, obj->psi + 61);
				} else if (obj->ps < 200) {
					atomic_set(&obj->ps_high_thd_val, obj->psi + 75);
					atomic_set(&obj->ps_low_thd_val, obj->psi + 43);
				} else {
					atomic_set(&obj->ps_high_thd_val, obj->psi + 50);
					atomic_set(&obj->ps_low_thd_val, obj->psi + 30);
				}
#else
...
static ssize_t stk3x1x_show_als(struct device_driver *ddri, char *buf)
{
	...
	// [wuzhe]
	if((res = stk3x1x_read_als(stk3x1x_obj->client, &val)))
		return scnprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
	else
		return scnprintf(buf, PAGE_SIZE, "0x%04X\n", val);
}
...
#if 0
                       if (obj->ps_cali < 50) {
                               atomic_set(&obj->ps_high_thd_val, obj->ps_cali + 101);
                               atomic_set(&obj->ps_low_thd_val,  obj->ps_cali + 61);
                       } else if (obj->ps_cali < 200) {
                               atomic_set(&obj->ps_high_thd_val, obj->ps_cali + 75);
                               atomic_set(&obj->ps_low_thd_val,  obj->ps_cali + 43);
                       } else if (obj->ps_cali < PS_MAX_THD_RANGE) {
                               atomic_set(&obj->ps_high_thd_val, obj->ps_cali + 50);
                               atomic_set(&obj->ps_low_thd_val,  obj->ps_cali + 30);
                       } else {
                               atomic_set(&obj->ps_high_thd_val, PS_THDH_LIMITED);
                               atomic_set(&obj->ps_low_thd_val,  PS_THDL_LIMITED);
                       }
...
/* [fengyunliang start] */
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
	atomic_set(&obj->psctrl_val, 0x32);
#else
...
static int __init stk3x1x_init(void)
{
	...
	if (node == NULL) {
		APS_ERR("dts node not found. node = NULL\n");
		return -EFAULT;
	}
