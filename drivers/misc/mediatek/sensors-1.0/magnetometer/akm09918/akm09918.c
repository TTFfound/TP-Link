...
static int AKECS_CheckDevice(void)
{
	...
	/* [fengyunliang start] Config hwinfo */
	if (akm_device == 0x05) {
		MAGN_PR_ERR("Device is AK09911\n");
		strcpy(akm_device_name, "AK09911");
	} else if (akm_device == 0x04) {
		MAGN_PR_ERR("Device is AK09912\n");
		strcpy(akm_device_name, "AK09912");
	} else if (akm_device == 0x10) {
		MAGN_PR_ERR("Device is AK09915\n");
		strcpy(akm_device_name, "AK09915");
	} else if (akm_device == 0x09) {
		MAGN_PR_ERR("Device is AK09916c\n");
		strcpy(akm_device_name, "AK09916c");
	} else if (akm_device == 0x0b) {
		MAGN_PR_ERR("Device is AK09916d\n");
		strcpy(akm_device_name, "AK09916d");
	} else if (akm_device == 0x0c) {
		MAGN_PR_ERR("Device is AK09918\n");
		strcpy(akm_device_name, "AK09918");
	} else {
		MAGN_PR_ERR("Device is unknown\n");
		strcpy(akm_device_name, "unknown");
	}
	/* [fengyunliang end] */
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,akm09918");
	if (node == NULL) {
		MAGN_PR_ERR("dts node not found. node = NULL\n");
		return -EFAULT;
	}
	...
	// [fengyunliang start] Porting sensors
	client->addr = AKM09918_I2C_ADDRESS;
	client->timing = 400;
	// [fengyunliang end]
	data->client = client;
