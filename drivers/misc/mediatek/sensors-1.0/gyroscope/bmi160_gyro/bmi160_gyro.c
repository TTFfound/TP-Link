		...
		enum BMG_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		...
		GYRO_LOG("get power status = 0x%x \n", data);
	}
	if (err < 0)
		...
		int datarate)
{
	...
	/* [wuzhe start] Fix data odr erroe */
	u8 rdata = 0;
	int i = 0;
	/* [wuzhe end] */
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		...
		/* [wuzhe start] Fix data odr error */
...
static int __init bmi160_gyro_init(void)
{
	...
	if (node == NULL) {
		GYRO_ERR("dts node not found. node = NULL\n");
		return -EFAULT;
	}
	...
	i2c_register_board_info(hw->i2c_num, &i2c_bmi160_gyro, 1);
