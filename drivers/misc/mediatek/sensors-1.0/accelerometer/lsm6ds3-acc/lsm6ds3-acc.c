...
static int LSM6DS3_init_client(struct i2c_client *client, bool enable)
{
	...
	// [wuzhe] modify scale from 2g to 4g
...
static int lsm6ds3_acc_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return lsm6ds3_set_delay((u64)samplingPeriodNs);
}
static int lsm6ds3_acc_flush(void)
{
...
/* [wuzhe start] */
static int lsm6ds3_step_c_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return lsm6ds3_step_c_set_delay((u64)samplingPeriodNs);
}
...
static int lsm6ds3_step_d_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return lsm6ds3_step_d_set_delay((u64)samplingPeriodNs);
}
...
static int lsm6ds3_floor_c_enable_nodata(int en)
{
	return 0;
}

static int lsm6ds3_floor_c_set_delay(u64 delay)
{
	return 0;
}

static int lsm6ds3_floor_counter_get_data(uint32_t *counter, int *status)
{
	return 0;
}
...
static int lsm6ds3_local_init(void)
{
	...
	else
	{
		...
		ctl.batch = lsm6ds3_acc_batch;
		ctl.flush = lsm6ds3_acc_flush;
...
static int lsm6ds3_step_c_local_init(void)
{
	...
	else
	{
		...
		step_ctl.enable_floor_c = lsm6ds3_floor_c_enable_nodata;
		step_ctl.floor_c_set_delay = lsm6ds3_floor_c_set_delay;

		/* [wuzhe start] */
		step_ctl.step_c_batch = lsm6ds3_step_c_batch;
		step_ctl.step_c_flush= lsm6ds3_step_c_flush;
		step_ctl.step_d_batch = lsm6ds3_step_d_batch;
		...
		step_data.get_data_floor_c = lsm6ds3_floor_counter_get_data;
...
static int __init lsm6ds3_init(void)
{
	...
	if (node == NULL) {
		GSE_ERR("dts node not found. node = NULL\n");
		return -EFAULT;
	}
