...
struct i2c_client *client, uint8_t addr, uint8_t *readbuf, int32_t readlen)
{
	...
	struct i2c_msg msg[2] = {
	{
		...
		.buf = buffer,
		.len = 2,
/* [fengyunliang start] Fix sensor freq */
...
/* [fengyunliang start] Acc Power should always on, if use step counter feature */
	...
	const char *buf, size_t count)
{
	...
	u8 acc_odr = 0, range = 0, acc_selftest_amp = 0, acc_selftest_sign = 0;
	struct bmi160_acc_i2c_data *client_data = obj_i2c_data;
...
static int bmi160_acc_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	return err;
#endif
    return bmi160_acc_set_delay((u64)samplingPeriodNs);
}
...
static int __init bmi160_acc_init(void)
{
	...
	GSE_FUN();
	i2c_register_board_info(hw->i2c_num, &i2c_bmi160_acc, 1);
