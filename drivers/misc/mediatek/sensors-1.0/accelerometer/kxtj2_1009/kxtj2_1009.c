...
/* [fengyunliang start] Config hwinfo */
static char kxtj_device_name[20] = "unknown";
/* [fengyunliang end] */
...
/*----------------------------------------------------------------------------*/
static struct data_resolution kxtj2_1009_data_resolution[2] = {
 /* combination by {FULL_RES,RANGE}*/
    {{ 0, 9}, 1024}, // dataformat +/-2g  in 12-bit resolution;  { 3, 9} = 3.9 = (2*2*1000)/(2^12);  256 = (2^12)/(2*2)
    {{ 1, 95}, 512}, // dataformat +/-4g  in 12-bit resolution;  // [wuzhe]
};
/*----------------------------------------------------------------------------*/
...
static int KXTJ2_1009_SetDataResolution(struct kxtj2_1009_i2c_data *obj)
{
	...
	obj->reso = &kxtj2_1009_data_resolution[1]; // [wuzhe]
...
static int KXTJ2_1009_CheckDeviceID(struct i2c_client *client)
{
	...
	/* [fengyunliang start] Config hwinfo */
	if (databuf[0] == 0x0F) {
		GSE_ERR("Device is KXTJ2_1009\n");
		strcpy(kxtj_device_name, "KXTJ2_1009");
	} else if (databuf[0] == 0x35) {
		GSE_ERR("Device is KXTJ3-1057\n");
		strcpy(kxtj_device_name, "KXTJ3-1057");
	} else {
		GSE_ERR("Device is unknown\n");
		strcpy(kxtj_device_name, "unknown");
	}
	/* [fengyunliang end] */
...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = KXTJ2_1009_SetDataFormat(client, KXTJ2_1009_RANGE_4G); // [wuzhe] modify data format from 2g to 4g
	if(res != KXTJ2_1009_SUCCESS) //0x2C->BW=100Hz
	{
		return res;
	}
...
{
	struct kxtj2_1009_i2c_data *obj = (struct kxtj2_1009_i2c_data*)obj_i2c_data;
	u8 databuf[20];
...
/*Kionix Auto-Cali Start*/
        ...
        if ( raw[2] >=0) 
            raw[2] = raw[2] * Sensitivity_def / abs(Z_AVG[0]); // Gain Compensation
        else 
            raw[2] = raw[2] * Sensitivity_def / abs(Z_AVG[1]); // Gain Compensation
...
/* [fengyunliang start] for hw info in /sys/kernel */
static ssize_t show_acc_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%d\n",
		kxtj_device_name, "Kionix", I2C_DRIVERID_KXTJ2_1009);
}
...
static int kxtj2_1009_create_attr(struct device_driver *driver) 
{
	...
	/* [fengyunliang add] for hw info in /sys/kernel */
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,kxtj2_1009");
	if (node == NULL) {
	...
	client->addr = KXTJ2_1009_I2C_SLAVE_ADDR;
	client->timing = 400;
