				...
				u8 *data, u8 len)
{
	...
	// [wuzhe]
	mdelay(1);/*delay 1ms for every write*/
	return err;
}
...
static int bmi160_floor_c_enable_nodata(int en)
{
	return 0;
}

static int bmi160_floor_c_set_delay(u64 delay)
{
	return 0;
}

static int bmi160_floor_counter_get_data(uint32_t *counter, int *status)
{
	return 0;
}
// [fengyunliang end]
...
// [wuzhe start]
static int bmi160_step_d_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return bmi160_step_d_set_delay((u64)samplingPeriodNs);
}

static int bmi160_step_c_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return bmi160_step_c_set_delay((u64)samplingPeriodNs);
}

static int bmi160_step_d_flush(void)
{
	return step_d_flush_report();
}

static int bmi160_step_c_flush(void)
{
...
/* [fengyunliang start] Overwrite the enable function */
static int bmi160_step_c_enable(int en)
{
	...
	if (en == 1) {
		...
		if (err < 0){
			STEP_C_ERR("bmi160_stc_get_mode failed.\n");
		}
...
static int bmi160_step_c_get_data(u32 *value, int *status)
{
	...
	if (0 == data  && valid_data_after_reset == 1) {
...
static int bmi160_stc_get_data_significant(u32 *value, int *status)
{
...
static int bmi160_stc_get_data_step_d(u32 *value, int *status)
{
...
static int step_c_i2c_probe(void)
{
	...
	// [wuzhe start]
	ctl.step_c_batch = bmi160_step_c_batch;
	ctl.step_d_batch = bmi160_step_d_batch;
	ctl.step_c_flush = bmi160_step_c_flush;
	ctl.step_d_flush = bmi160_step_d_flush;
	// [wuzhe end]
	...
	data.get_data_floor_c = bmi160_floor_counter_get_data;
	// [fengyunliang end]
	...
	step_c_init_flag = 0;
	STEP_C_LOG("%s: is ok.\n", __func__);
	/* [fengyunliang start] Auto start step counter, and keep it running */
