...
/* [lichenggang start]add for 913*/
DEFINE_MUTEX(himax_mutex_lock);
extern atomic_t close_incall;//[lichenggang start] modify for psensor contorl tp when call
extern atomic_t double_enable;//[lichenggang start] add for gesture
atomic_t interrupt_counter = ATOMIC_INIT(0); /* [lichenggang] add tp report rate node */
/* [lichenggang end] */
...
void setSelfMaxBuffer(void)//morgen
{
    I("%s:\n", __func__);
    diag_self_max = kzalloc((x_channel + y_channel) * sizeof(int32_t), GFP_KERNEL);
}
void setMutualMaxBuffer(void)//morgen
{
    I("%s:\n", __func__);
    diag_mutual_max = kzalloc(x_channel * y_channel * sizeof(int32_t), GFP_KERNEL);
}
/* [yanlin end] */
...
/* [yanlin start] TP threshold test */
void himax_get_mutual_max(void)//morgen
{
    int i = 0;
    for(i = 0; i < (x_channel * y_channel); i++)
    {
        if(diag_mutual[i] > diag_mutual_max[i])
            diag_mutual_max[i] = diag_mutual[i];
    }
}
/* [yanlin end] */
...
/* [yanlin start] TP threshold test */
static void himax_diag_max_arrange_print(struct seq_file *s, int i, int j, int transpose)//morgen
{
    if (transpose) {
        seq_printf(s, "%6d", diag_mutual_max[j + i*x_channel]);
    } else {
        seq_printf(s, "%6d", diag_mutual_max[i + j*x_channel]);
    }
}
/* [yanlin end] */
...
static void himax_diag_arrange_inloop(struct seq_file *s, int in_init,int out_init,bool transpose, int j)
{
    ...
    if (in_init > 0) //bit0 = 1
    {
        ...
        {
            /* [yanlin start] TP threshold test */
            if (IS_MAX_RAWDATA)//morgen
                himax_diag_max_arrange_print(s, i, j, transpose);
            else
...
static ssize_t himax_diag_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
    ...
    /* [yanlin start] TP threshold test */
    IS_MAX_RAWDATA = false;//morgen

    memset(receive, 0x00, sizeof(receive));
    memset(diag_mutual_max, 0x00, x_channel * y_channel * sizeof(int32_t));//morgen
    /* [yanlin end] */
    ...
    if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8)
    {
        ...
        /* [yanlin start] TP threshold test */
        else if (g_diag_command == 0x09) {//morgen
            command[0] = 0x01;
        }
        else {
            command[0] = g_diag_command;
    ...
    else if (((storage_type == 9)||(storage_type > 0 && storage_type < 8)) && (rawdata_type > 0 && rawdata_type < 8)) // [yanlin] TP threshold test
    {
        I("%s,dsram\n",__func__);

        /* [yanlin start] TP threshold test */
        if (storage_type == 9) {//morgen
            IS_MAX_RAWDATA = true;
            storage_type = 1;
            g_diag_command = storage_type * 10 + rawdata_type;
        }
        /* [yanlin end] */
...
/* [lichenggang start] modify for self_test node */
static int tp_self_test(struct seq_file *m, void *v)
{
    int val=0x00;
    ssize_t ret = 0;
    ...
    if (val == 0x01)
    {
        seq_printf(m, "%s", "Self_Test Pass\n");
    }
    else
    {
        seq_printf(m, "%s", "Self_Test Fail\n");
    }
    g_self_test_entered = 0;
    himax_ic_reset(true, false);/* [lichenggang] reload config file */
    return ret;
}

static int himax_self_test_read(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, tp_self_test, NULL);
}
/* [lichenggang end] */
...
{
    .owner = THIS_MODULE,
    .read = seq_read, //[lichenggang] add for self_test node
    .owner = THIS_MODULE,
    .open = himax_self_test_read, //[lichenggang] add for self_test node
};
#endif
...
#ifdef HX_SMART_WAKEUP
static int himax_SMWP_read(struct seq_file *m, void *v)
{
    size_t ret = 0;
    E("enter::%s\n",__func__);
...
static int himax_SMWP_open(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, himax_SMWP_read, NULL);
}
...
static struct file_operations himax_proc_SMWP_ops =
{
    ...
    .open = himax_SMWP_open,
};

/* [lichenggang start] add for IC support gesture ID */
static int himax_support_gesture_show(struct seq_file *m, void *v)
{
    size_t ret = 0;
    E("enter::%s\n",__func__);
    ret = seq_printf(m, "0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",0x01, 0x02,
                    0x04, 0x05, 0x06, 0x07, 0x0C, 0x0D);
    return ret;
}

static int himax_support_gesture_open(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, himax_support_gesture_show, NULL);
}

static struct file_operations himax_proc_support_gesture_ops =
{
    .owner = THIS_MODULE,
    .read = seq_read,
    .open = himax_support_gesture_open,
};
/* [lichenggang end] */
#endif
...
#ifdef HX_SMART_WAKEUP
extern  int himax_gesture_coordinate_show(struct seq_file *m, void *v);
static int himax_gesture_coordinate_open(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, himax_gesture_coordinate_show, NULL);
}
static struct file_operations himax_proc_gesture_coordinate_ops =
{
    .owner = THIS_MODULE,
    .read = seq_read,
    .open = himax_gesture_coordinate_open,
};
#endif
/* [lichenggang end] */


#ifdef PSENSOR_CONTROL_TP_ENABLE
...
static ssize_t close_incall_write_func(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
 {
	...
	mutex_lock(&himax_mutex_lock);
	if((ret==1)||(ret==0))
	{
		atomic_set(&close_incall,ret);
	}
	pr_err("%s:close_incall=%d\n",__func__,atomic_read(&close_incall));
	mutex_unlock(&himax_mutex_lock);
	return count;
}

static int close_incall_show(struct seq_file *m, void *v)
{

	size_t ret = 0;
	E("enter::%s\n",__func__);
	ret += seq_printf(m, "%d\n", atomic_read(&close_incall));
	return ret;
}

static int close_incall_open(struct inode *inode, struct file *file) {
	I("%s: enter, %d \n", __func__, __LINE__);
	return single_open(file, close_incall_show, NULL);
}

static const struct file_operations close_incall_fops = {
	.write = close_incall_write_func,
	.read = seq_read,
	.open = close_incall_open,
	.owner = THIS_MODULE,
};
/* [lichenggang end] */
#endif
...
/* [lichenggang start] add tp report rate node*/
static int tp_report_rate_show(struct seq_file *m, void *v)
{
...
static int tp_report_rate_open(struct inode *inode, struct file *file) {
	I("%s: enter, %d \n", __func__, __LINE__);
	return single_open(file, tp_report_rate_show, NULL);
}

static const struct file_operations tp_report_rate_fops = {
	.owner = THIS_MODULE,
	.read = seq_read,
	.open = tp_report_rate_open
};
/* [lichenggang end] */
...
/* [lichenggang start]add for tp_info node */
u8 firmware_version = 0;
#define TP_INFO_PAGE_SIZE        512
ssize_t himax_touch_tp_info_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char panel_type[] = "truly";
    unsigned char panel_ic[] = "hx83102B";
    unsigned char  driver_version[] = "1.0";
    u32 nLength = 0;

    mutex_lock(&himax_mutex_lock);
    himax_read_FW_ver(private_ts->client);
    himax_ic_reset(true, false);/* [lichenggang] reset IC */
    himax_int_enable(private_ts->client->irq, 1);/* [lichenggang] enable irq */
...
static DEVICE_ATTR(tp_info, S_IRUGO, himax_touch_tp_info_read, NULL);

int himax_touch_tp_info_init(void)
{
    if (sysfs_create_file(kernel_kobj, &dev_attr_tp_info.attr)) {
        E("hx83102B::sysfs_create_file tp_info fail\n");
        return -ENODEV;
    }
    return 0;
}
...
/* [lichenggang end] */
    ...
    himax_proc_support_gesture_file = proc_create(HIMAX_PROC_SUPPORT_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
                                          himax_touch_proc_dir, &himax_proc_support_gesture_ops);
    if(himax_proc_support_gesture_file == NULL)
    {
...
/* [lichenggang start] add for gesture coordinate */
#ifdef HX_SMART_WAKEUP
    himax_proc_gesture_coordinate_file = proc_create(HIMAX_PROC_GESTURE_COORDINATE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
                                            himax_touch_proc_dir, &himax_proc_gesture_coordinate_ops);
    if(himax_proc_support_gesture_file == NULL)
    {
        E(" %s: proc GESTURE file create failed!\n", __func__);
        goto fail_15;
...
#ifdef PSENSOR_CONTROL_TP_ENABLE
    /* [lichenggang start] modify for psensor contorl tp when call*/
    himax_proc_psensor_disable_tp = proc_create(HIMAX_PROC_PSENSOR_DISABLE_TP_FILE,(S_IWUSR|S_IRUGO|S_IWUGO),
                                            himax_touch_proc_dir,&close_incall_fops);
    if(himax_proc_psensor_disable_tp == NULL)
    {
        E(" %s: proc psensor_disable_tp file create failed!\n", __func__);
        goto fail_15;
    }
    /* [lichenggang end] */
#endif
...
/* [lichenggang start] add tp report rate node*/
himax_proc_tp_report_rate = proc_create(HIMAX_PROC_TP_RATE_REPORT_FILE,(S_IWUSR|S_IRUGO|S_IWUGO),
                                        himax_touch_proc_dir,&tp_report_rate_fops);
if(himax_proc_tp_report_rate == NULL)
{
    E(" %s: proc himax_proc_tp_report_rate file create failed!\n", __func__);
    return -ENOMEM;;
}
/* [lichenggang end] */
...
#endif
#ifdef HX_SMART_WAKEUP
    remove_proc_entry( HIMAX_PROC_SUPPORT_GESTURE_FILE, himax_touch_proc_dir );
fail_15:
