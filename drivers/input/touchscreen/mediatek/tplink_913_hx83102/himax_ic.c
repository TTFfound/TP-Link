...
void himax_touch_information(struct i2c_client *client)
{
    ...
    if(IC_TYPE == HX_83102A_SERIES_PWON)
    {
        ...
        //I("data[0] & 0x01 = %d\n",(data[0] & 0x01));
        if((data[1] & 0x01) == 1) /* [lichenggang] change data[0] to data[1], for himax default driver is err */
        {
...
extern u8 firmware_version;//[lichenggang]add for tp_info node
...
void himax_read_FW_ver(struct i2c_client *client)
{
    ...
    firmware_version = data[2];//[lichenggang]add for tp_info node
...
void himax_resend_cmd_func(bool suspended)
{
    ...
    //himax_set_SMWP_enable(ts->client,ts->SMWP_enable,suspended);/* [lichenggang] Close gesture when resume */
#endif
#ifdef HX_HIGH_SENSE
    himax_set_HSEN_enable(ts->client,ts->HSEN_enable,suspended);
#endif
#ifdef HX_THRESHOLD_ADJUST_ENABLE
    himax_set_sensitivity_enable(ts->client,ts->sensitivity);/* [lichenggang] add for sensitivity adjustment */
#endif
#ifdef HX_USB_DETECT_GLOBAL
