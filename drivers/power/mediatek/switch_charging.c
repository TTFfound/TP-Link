...
#include <bq2560x.h> // [wuzhe] Add for exhibition mode support
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	/* [zhangdong start] Modify CV according temp */
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	/* [zhangdong] Modify battery temp standard */
...
static void set_jeita_charging_current(void)
{
	...
	/* [zhangdong start] Config charging current according temp */
	if (g_temp_status == TEMP_BELOW_POS_0) {
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	/* [zhangdong start] ignore thermal when screen off */
#ifdef CONFIG_TPLINK_PRODUCT_TP913
...
void select_charging_current(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			/* [zhangdong start] Modify tp910 charging current according charger type*/
			g_temp_input_CC_value = batt_cust_data.ac_charger_current;
...
#else
	if ((g_bcct_flag == 1) && (g_temp_CC_value > (g_bcct_value * 100)))      // [zhangdong] Modify bat charging current
		g_temp_CC_value = g_bcct_value * 100;
	if ((g_bcct_input_flag == 1) && (g_temp_input_CC_value > (g_bcct_input_value * 100)))  // [zhangdong] Modify chr input current
...
static void mtk_select_ichg_aicr(void)
{
	...
	/* [zhangdong start] call state into consideration */
#ifdef CONFIG_TPLINK_PRODUCT_TP913
	...
	/* [zhangdong] disable charging for runin test */
	if (get_Charging_Mode() == 0 && get_Runin_Test_Mode() == true) {
		enable_charger = 2;
		battery_log(BAT_LOG_CRTI,
				"[BATTERY] runin test, turn off charging !\r\n");
	} else if (g_temp_CC_value > 0 && g_temp_input_CC_value > 0)
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_exhibition_mode != 0) {
			if (g_temp_status == TEMP_ABOVE_POS_60) {
				...
				bq2560x_set_vrechg(0x1);	/* VRECHG 0.2V */
			} else if (g_temp_status == TEMP_POS_10_TO_POS_45) {
				bq2560x_set_vrechg(0x1);	/* VRECHG 0.2V */
			} else if (g_temp_status == TEMP_POS_0_TO_POS_10) {
				bq2560x_set_vrechg(0x1);	/* VRECHG 0.2V */
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if ((charging_full_check() == KAL_FALSE) || (g_exhibition_mode != 0)) { // [wuzhe add]
...
{
	unsigned int charging_enable;  /* [zhangdong] disable charging for runin test */
