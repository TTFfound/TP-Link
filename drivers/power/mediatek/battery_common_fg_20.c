...
unsigned int g_screen_state = SCREEN_OFF;      // [zhangdong] Add screen state flag
...
/* [zhangdong start] disable high temp protect */
...
/* [zhangdong start] Add shipping mode */
...
/* [zhangdong start] Disable charging for Runin test */
int Charging_Mode = 1;
...
static int sleep_total_sleep = 0; // [houjihai] Fix battery soc update slowly
...
/* CC: modify to have a full power supply status */
	...
	.BAT_CAPACITY = -1,
#else
	.BAT_CAPACITY = -1,           // [zhangdong] Modify bat_capacity default value for test
#endif
	.BAT_batt_vol = 0,
	.BAT_batt_temp = 0,
	/* Dual battery */
...
/* [zhangdong start] Modify call and screen state charging current */
...
/* [zhangdong start] Add shipping_mode */
...
#endif
			Charging_Mode = 1;   // [zhangdong] disable charging for runin test
		}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_ScreenState);  // [zhangdong] screen state sysfile
...
static void battery_timer_resume(void)
{
	...
	/* [houjihai start] Fix battery soc update slowly */
	int is_total_time = 0;
	static struct timespec pre_bat_time_after_sleep;
	/* [houjihai end] */
	...
	/* [houjihai start] Fix battery soc update slowly */
	sleep_total_sleep += (bat_time_after_sleep.tv_sec - pre_bat_time_after_sleep.tv_sec);
	pre_bat_time_after_sleep.tv_sec = bat_time_after_sleep.tv_sec;
	battery_log(BAT_LOG_CRTI, "sleep_total_sleep: %d\n", sleep_total_sleep);
	if (sleep_total_sleep >= 4) {
		sleep_total_sleep = 0;
		is_total_time = 1;
	}
	/* [houjihai end] */
	...
	if (is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout || battery_meter_get_low_battery_interrupt_status() || is_total_time) { // [houjihai] Fix battery soc update slowly
		mutex_lock(&bat_mutex);
		battery_meter_reset_sleep_time();
		BAT_thread();
		mutex_unlock(&bat_mutex);
...
static int mtk_tpcharger_setup_files(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_err("%s: Batt_Temp_protect devices create file error!\n", __func__);
		goto _out;
	}
	...
	if (ret) {
		pr_err("%s: Charging_Mode devices create file error!\n", __func__);
		goto _out;
	}
	/* [zhangdong end] */
_out:
