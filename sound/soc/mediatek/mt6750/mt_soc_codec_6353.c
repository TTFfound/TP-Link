...
/* [wuhefeng start] product distinguish of analog switch */
...
/* wuhefeng start */
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
static int Audio_Analog_Switch_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] = mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL];
    return 0;
}

static int Audio_Analog_Switch_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    if (ucontrol->value.integer.value[0]) {
        AudDrv_GPIO_EXTAMP2_Select(true, 1);
    } else {
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	...
	if (enable) {
		...
		mHPSPKon = 1;
		printk("AudDrv_GPIO_EXTAMP2_Select enable.......\n");
		AudDrv_GPIO_EXTAMP2_Select(true, 1);
		/* HP output swtich release to normal output */
		if (mInTrimming == 0) /* Not do when triming to avoid pop */
		HP_Ana_Switch_to_Release();
...
#endif
		printk("AudDrv_GPIO_EXTAMP2_Select false.......\n");
		AudDrv_GPIO_EXTAMP2_Select(false, 1);
		/*Ana_Set_Reg(MT6353_CLK_CKPDN_CON2_SET, 0x0003, 0xffff); MT6328*/
		Ana_Set_Reg(MT6353_CLK_CKPDN_CON3_SET, 0x00C0, 0xffff); /*MT6353*/
		/* Disable ClassAB/D clock */
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function), //wuhefeng add
#endif
};
...
static const struct snd_kcontrol_new mt6331_snd_controls[] = {
    ...
    /* wuhefeng start */
#if defined(CONFIG_TPLINK_PRODUCT_TP913)
    SOC_ENUM_EXT("Audio_Analog_Switch", Audio_DL_Enum[13],
             Audio_Analog_Switch_Get,
             Audio_Analog_Switch_Set),
#endif
    /* wuhefeng end */
};
